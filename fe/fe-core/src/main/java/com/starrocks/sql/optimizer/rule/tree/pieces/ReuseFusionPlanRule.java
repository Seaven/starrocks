...
public class ReuseFusionPlanRule implements TreeRewriteRule {
    ...
    private final Map<Integer, QueryPiecesPlan> fusionPiecsMap = Maps.newHashMap();
    ...
    public OptExpression rewrite(OptExpression root, TaskContext taskContext) {
        ...
        transformer.transformSPJGPieces(root);
    ...
    private OptExpression generateCTEPlan(CTEContext cteContext, OptExpression root) {
        ...
        for (QueryPiecesPlan plan : fusionPiecsMap.values()) {
    ...
    private OptExpression rewritePlan(OptExpression root) {
        ...
        if (!fusionPiecsMap.containsKey(pieces.getPiece().planId)) {
        ...
        QueryPiecesPlan fusionPlan = fusionPiecsMap.get(pieces.getPiece().planId);
        for (ColumnRefOperator originRef : outputs.getColumnRefOperators(factory)) {
            cteRefsMapping.put(originRef, fusionPlan.columnRefConverter.convert(originRef));
        }
    ...
    private void recommendFusionCTE(List<QueryPiecesPlan> piecesPlans) {
        ...
        for (Map.Entry<String, List<QueryPiecesPlan>> entry : groups.entrySet()) {
            ...
            p.ifPresent(queryPiecesPlan -> pieces.forEach(pi -> fusionPiecsMap.put(pi.planId, queryPiecesPlan)));
        }
    }
    ...
    private static class PiecesFusion extends OperatorVisitor<Optional<QueryPieces>, List<QueryPieces>> {
            ...
            for (QueryPieces pieces : context) {
                ...
                p.getColumnRefMap().forEach((k, v) -> {
                    if (converter.contains(k)) {
                        project.put(converter.convert(k), converter.convert(v));
                    } else {
        ...
        public Optional<QueryPieces> visitLogicalAggregation(LogicalAggregationOperator node,
            ...
            List<ScalarOperator> pieceFilters = piecePlans.stream()
                    .map(p -> converter.convert(Utils.compoundAnd(p.pieceFilters)))
                    .collect(Collectors.toList());
            ...
            List<ColumnRefOperator> groupBys = node.getGroupingKeys().stream().map(converter::convert)
                    .collect(Collectors.toList());
            List<ColumnRefOperator> partitions = node.getPartitionByColumns().stream().map(converter::convert)
                    .collect(Collectors.toList());
        ...
        public Optional<QueryPieces> visitLogicalJoin(LogicalJoinOperator node, List<QueryPieces> context) {
            if (context.stream().map(p -> p.normalizedOp).distinct().count() > 1) {
                return Optional.empty();
            }
        ...
        public Optional<QueryPieces> visitLogicalTableScan(LogicalScanOperator node, List<QueryPieces> context) {
            ...
            Set<ScalarOperator> fusionPredicate = Sets.newHashSet();
            for (QueryPieces piece : context) {
                ...
                fusionPredicate.add(newPredicate);
            }
            ScalarOperator predicate = Utils.compoundOr(fusionPredicate);
