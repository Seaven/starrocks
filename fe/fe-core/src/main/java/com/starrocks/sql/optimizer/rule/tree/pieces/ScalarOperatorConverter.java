...
class ScalarOperatorConverter {
    ...
    private final ReplaceColumnRefRewriter rewriter = new ReplaceColumnRefRewriter(refsMapping, false);
    ...
    public ColumnRefOperator convert(ColumnRefOperator columnRef) {
        if (refsMapping.containsKey(columnRef)) {
            return refsMapping.get(columnRef);
        }

        Preconditions.checkState(nextID > 0);
        return convert(columnRef, nextID++);
    }

    public ColumnRefOperator convert(ColumnRefOperator originRef, int newId) {
    ...
    public ScalarOperator convert(ScalarOperator operator) {
        return rewriter.rewrite(operator);
    }
}
